"""Benchmark persistent BM256/BN256 with grouped SQMMA waits."""
import argparse
import importlib
...
def _consumer(reader, c_ptr, pid, M, N, stride_cm, stride_cn,
              ...
              k_tiles: tl.constexpr, mma_group: tl.constexpr):
    for tile_iter in range(tile_iters):
        tile_id = pid + tile_iter * num_sms
        if tile_id < total_tiles:
            pid_m = tile_id // grid_n
            ...
            for group_start in tl.static_range(0, k_tiles, mma_group):
                for group_offset in tl.static_range(mma_group):
                    token = tile_iter * k_tiles + group_start + group_offset
                    ready = reader.wait(token)
                    acc = tle.gpu.wgmma(ready.slot.a, ready.slot.b, acc)
                acc = tle.gpu.wgmma_wait(0, acc)
                for group_offset in tl.static_range(mma_group):
                    reader.release(tile_iter * k_tiles + group_start + group_offset)
...
@triton.jit
def persistent_group_kernel(a_desc, b_desc, c_ptr, M, N, stride_cm, stride_cn,
    ...
    b_smem = tle.gpu.alloc([NUM_SLOTS, 64, 256], dtype=b_desc.dtype,
                           layout=None, scope=tle.gpu.smem,
                           nv_mma_shared_layout=True)
    pipe = tle.pipe(capacity=NUM_SLOTS, scope="cta", name="probe_persistent_group",
                    a=a_smem, b=b_smem)
...
def main():
    ...
    ap.add_argument("--slots", type=int, choices=(2, 3), default=3)
    ap.add_argument("--mma-group", type=int, choices=(1, 2, 3, 4), default=2)
    ap.add_argument("--warmup", type=int, default=5)
    ap.add_argument("--rep", type=int, default=20)
    args = ap.parse_args()
    m, n, k = args.shape
    if k % 64 or n % 256 or (k // 64) % args.mma_group:
        ap.error("K/N alignment or K tile divisibility invalid")
    ...
    torch.musa.synchronize()
    t_ms = float(start.elapsed_time(end)) / args.rep
    print(f"RESULT shape={m}x{n}x{k} dtype={args.dtype} mma_group={args.mma_group} "
          f"num_sms={args.num_sms} slots={args.slots} torch_ms={torch_ms:.6f} "
          f"probe_ms={t_ms:.6f} ratio={torch_ms/t_ms:.4f}", flush=True)
