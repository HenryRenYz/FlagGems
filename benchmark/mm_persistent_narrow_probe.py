"""Benchmark a persistent MTT GEMM with a descriptor-matched narrow N tile.

The production multifield persistent kernel is fixed at ``BN=256``.  This
probe uses ``BM=256, BN=64, BK=64`` so N=64 workloads do not over-read a
256-column descriptor.  It is intentionally benchmark-only until an
independent MTT run establishes a stable latency and correctness result.
"""
    ...
    for tile_iter in range(tile_iters):
        ...
        if tile_id < total_tiles:
            ...
            for k_iter in range(k_tiles):
                ...
                tle.gpu.copy(b_desc, slot.b, [64, 64], [k_offset, n_offset])
                writer.commit(token)
...
def _consumer(
    ...
    for tile_iter in range(tile_iters):
        ...
        if tile_id < total_tiles:
            ...
            m_offset = (pid_m * 256).to(tl.int32)
            n_offset = (pid_n * 64).to(tl.int32)
            acc = tl.zeros((256, 64), dtype=tl.float32)
            for k_iter in range(k_tiles):
                token = tile_iter * k_tiles + k_iter
            ...
            offs_n = n_offset + tl.arange(0, 64)
...
def main():
    ...
    m, n, k = args.shape
    if n != 64:
        ap.error("narrow persistent probe requires N == 64")
    if k % 64:
        ap.error("K must be divisible by 64")
    if args.num_sms <= 0:
        ap.error("num-sms must be positive")
    ...
    db = TensorDescriptor.from_tensor(b, [64, 64])

    def run():
        with torch_device_fn.device(a.device):
            persistent_narrow_kernel[(args.num_sms,)](
                da,
                db,
                out,
                m,
    ...
    print(
        f"RESULT shape={m}x{n}x{k} dtype={args.dtype} bm=256 bn=64 bk=64 "
        f"num_sms={args.num_sms} slots={args.slots} tiles={total_tiles} "
        f"tile_iters={tile_iters} torch_ms={torch_ms:.6f} "
        f"probe_ms={probe_ms:.6f} ratio={torch_ms/probe_ms:.4f}",
        flush=True,
    )
